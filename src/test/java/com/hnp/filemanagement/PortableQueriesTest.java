...
import com.hnp.filemanagement.folder.persistence.TagRepository;
 ...
 *
 * <p><b>On MySQL these passed with or without the {@code UPPER} the queries carry</b>, because its
 * {@code utf8mb4_unicode_ci} collation already compared that way - which was exactly the trap:
 * nothing on MySQL showed that a query depended on the collation. They were written for release B,
 * which ran the suite on both databases; on PostgreSQL, the only one since release C, a bare
 * {@code =} or {@code LIKE} fails them.
 ...
 * ({@link SearchKey}), so no collation helps them and these tests prove the fold itself. A search is handed its term folded, as the services do -
...
class PortableQueriesTest extends DatabaseSupport {
     ...
     * every list page with nothing typed in it (issue 87). MySQL accepted either, which is why it
     * went unnoticed until the suite ran on PostgreSQL.
     */
    @Test
