...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    // The same four, leaving one account out: what an edit asks, since the account being edited
    // holds its own values and must not count as the duplicate of itself (issue 88).

    boolean existsByUsernameIgnoreCaseAndIdNot(String username, Integer id);

    boolean existsByPersonelCodeAndIdNot(Integer personelCode, Integer id);

    boolean existsByNationalCodeAndIdNot(String nationalCode, Integer id);
