...
import com.hnp.filemanagement.identity.security.ActiveUserSessions;
...
public class UserService {
    ...
    public void updateUser(UserDTO userDTO, int principalId) {
        ...
        requireNoDuplicateOfAnother(user.getId(), newUsername, newPersonelCode, newNationalCode, newPhoneNumber,
                userDTO);
    ...
    /**
     * The same checks for an edit, with the edited account left out (issue 88). A value counts as
     * changed when it differs exactly - so {@code ali} to {@code Ali} is a change - while the
     * username is taken when another account holds it without regard to case. Without leaving the
     * account out, the only account holding {@code ali} was the one being renamed, and the rename
     * was refused as a duplicate of itself.
     */
    private void requireNoDuplicateOfAnother(int userId, String username, Integer personelCode,
                                             String nationalCode, String phoneNumber, UserDTO context) {

        boolean duplicate =
                (username != null && userRepository.existsByUsernameIgnoreCaseAndIdNot(username, userId))
                        || (personelCode != null && userRepository.existsByPersonelCodeAndIdNot(personelCode, userId))
                        || (nationalCode != null && userRepository.existsByNationalCodeAndIdNot(nationalCode, userId))
                        || (phoneNumber != null && userRepository.existsByPhoneNumberAndIdNot(phoneNumber, userId));
