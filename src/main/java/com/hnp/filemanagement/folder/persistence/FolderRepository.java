...
public interface FolderRepository extends JpaRepository<Folder, Integer> {
     ...
     * enforce "only one row with a null parent" — a unique index treats nulls as distinct —
    ...
    @Query("""
            SELECT new com.hnp.filemanagement.folder.persistence.ChildCount(f.tagGroup.id, COUNT(f.id))
            FROM Folder f
     ...
     */
    @Query("""
            SELECT new com.hnp.filemanagement.folder.persistence.ChildCount(f.parent.id, COUNT(f.id))
            FROM Folder f
     ...
     */
    @Query("""
            SELECT new com.hnp.filemanagement.folder.persistence.GrantedPath(g.folder.path, g.permission)
